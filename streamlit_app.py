...
def create_weather_visualization(df: pd.DataFrame, windows: List[Dict], thresholds: Dict, text_color: str):
    ...
    fig.update_yaxes(title_text="Comfort Score", row=1, col=1, secondary_y=True)
    fig.update_yaxes(title_text="Precipitation (mm) / Wind (m/s)", row=2, col=1)
    fig.update_yaxes(title_text="UV Index / Cloud Cover (%)", row=3, col=1)
...
if compute:
    ...
    with st.spinner("Fetching historical data..."):
        try:
            ...
            current_year = dt.datetime.now().year
            historical_start = f"{current_year - 5}-{start_date.strftime('%m-%d')}"
            historical_end = f"{current_year - 1}-{end_date.strftime('%m-%d')}"
            
            nasa_data = nasa_power_daily_point(
                st.session_state.lat, st.session_state.lon, 
            ...
            if nasa_data:
                ...
                st.success(" Using NASA POWER historical data")
